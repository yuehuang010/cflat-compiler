...
- **Object-like** `#define NAME <expr>` is *folded to a constant* by clang and surfaced as a bare global with its natural C type (the paragraph above). If the body does not fold to an integer / float / string / pointer constant - it expands to a token paste, or to an expression over names CFlat does not have - the macro is skipped and the name simply does not exist.
- **Alias** `#define NAME OTHER`, an object-like macro whose entire body is a single identifier, is *bound onto whatever `OTHER` names*: a C function (`#define GetPerformanceInfo K32GetPerformanceInfo` makes `GetPerformanceInfo(...)` callable), an `extern` global (the alias shares the target's storage), a struct tag, a function-like macro (the alias becomes a second generic wrapper), or another alias - chains are followed. A body that still folds to a constant keeps the constant (`#define A B` where `B` is `#define B 100` gives `A == 100`). An alias to a name CFlat does not know is dropped silently, and an alias whose own name is already a declared function or global never shadows it.
