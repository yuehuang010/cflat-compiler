...
| `--force` | `-B` | | Rebuild a `-o` executable even when its dependency manifest says it is up to date. |
...
Full details, including the `import package` / inline `lib`/`define` clauses, are in
...
| `--error-on-cpp-reparse` | Verify the C/C++ translation-unit cache (`cheaders/`). Every TU the compile touches - a C++ header import group, a C header import, an imported `.c`/`.cpp` file's signature extraction, and its object compile - counts each full clang parse, and the compile fails when any TU is parsed more than once. A cache hit is not a parse, so the same check holds whether the cache is cold or warm. The error names the TU and its count. With `-v`, every counted parse prints one `[verbose] clang parse: <kind> '<tu>' #<n> (<stage>)` line, so `grep "clang parse:"` counts them. Without the switch, the `CFLAT_CPP_MAX_HEADER_PARSES=<n>` environment variable sets the same per-TU budget. `test.bat` / `test.sh` / `test_err.bat` pass it on every compile. |
...
| `--locale` | | locale | Select the compiler diagnostic locale, named after a catalog in `--locale-dir` (`de`, `en`, `es`, `fr`, `it`, `ja`, `ko`, `ru`, `zh-Hans`, `zh-Hant`). Defaults to `en`; `CFLAT_LOCALE` is used when omitted. Use `pseudo` to print source templates and coverage warnings. |
...
remains in compiler source and is the fallback; the default catalog is `en`. Catalog keys are generated by lowercasing the
...
prints source templates and reports whether each `en` key is present. The test
discovery pass writes the source-template catalog to `en-pseudo.json`; this is separate
from the default `en.json` catalog.
...
`zh-Hant`), then by primary subtag, falling back to `en`. `CFLAT_LOCALE` overrides
