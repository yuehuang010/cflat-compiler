...
int main(int argc, char* argv[])
{
    ...
    args.addMultiOption("define", 'D', "Define a global compile-time macro readable by `if const` and by expressions: -DNAME (=1) or -DNAME=value. A numeric value becomes an int macro, anything else a string macro. A later -D of the same name overwrites an earlier one. Builtin macros (__PLATFORM__, __WINDOWS__, ...) cannot be redefined, and .cb sources can neither add nor change a define");
