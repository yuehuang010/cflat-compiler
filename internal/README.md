...
| [macos-build.md](macos-build.md) | macOS arm64 native build internals: `ld64.lld` link path, self-contained tbd-stub harvesting via `--init`, `vcpkg.json` RTTI/lld rationale, Darwin runtime `if const (__MACOS__)` specifics, `--run` on Mach-O; moved from CLAUDE.md |
| [testing-notes.md](testing-notes.md) | `test.sh` SKIP-list discipline + the `--init` warm-cache second pass (serializer round-trip rule for `TypeAndValue`/`StructData`/`AnnotationValue`); moved from CLAUDE.md |
