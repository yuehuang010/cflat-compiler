...
(`test.sh Release`, `test_example.sh`, `test_lsp.sh`) unless it lists an intended, enumerated test
...
- Verify (met): `move Holder` / `move string` into a plain param both error; test.sh 468/0,
  test_example.sh 35/0, test_lsp.sh 152/0 - zero false positives from the wider predicate.
...
- Verify (met): gates a-e all pass; early-return leak repro is leaks=0; test.sh 470/0 (cold AND warm),
  test_example.sh 35/0, test_lsp.sh 152/0.
...
- Verify: per-container matrix; `test.sh` + `test_example.sh` green (examples exercise these heavily).
- Depends: 4.
...
- Verified: `test.sh` 472/0 (cold+warm), `test_example.sh` 35/0, `test_lsp.sh` pass.
