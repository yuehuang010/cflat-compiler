...
| 10 | `p3/alloc-alignment-unclaused-global-pointer-store-unchecked` | LANDED 2026-09-04 `fix/align-doors` (global store door, both directions; the clause-bearing direction fires only on PRECISE source provenance) | add the GLOBAL destination to the `RejectFieldAllocAlignMismatch` door in ParseAssignmentExpression; optional address-of door closes the `E** pp = &aligned` launder in the same rule. Legs in err_align_alloc_mismatch.cb + an accept leg for a declared `alignas(0,64) E*` global. |
| 11 | `p3/explicit-unique-align-spelling-rejects-plain-new` | LANDED 2026-09-04 `fix/align-doors` (ALIGN generic argument folded onto AllocAlignValue in BOTH ParseDeclarationSpecifiers copies) | seed `pendingInitAllocAlign` from the `unique<T, N>` template argument as the sugar path does (declaration + assignment doors); reword the mismatch message to name the declared type. |
| 12 | `p3/mixed-owning-borrow-pointer-ternary-join-leaks-new-arm` | LANDED 2026-09-04 `fix/align-doors` (proven-owning vs proven-borrow pointer join rejected; four test_move legs retargeted to err_ternary_mixed_ownership.cb) | extend `RejectMixedOwnershipTernaryJoin` to pointer joins where one arm is `new` / `move` / owning result and the other a borrow. Measure blast radius (`? new ` / `: new ` in Test/ and example/) and check LANGUAGE.md's pointer-join text first. |
