...
**Recount 2026-08-08, after `fix/cast-launder-occurrence` landed:**
[[same-statement-cast-launders-join-code-evidence]] is FIXED and its file DELETED (occurrence
keying - see the file's own "Fix direction", now implemented on `codeValueDataCasts_` /
`dataValueCodeCasts_`). Review round 1 filed two residuals the fix's argument-granularity keying
cannot reach: [[join-arm-cast-launders-sibling-arm]] (both arms of one join share an occurrence,
so a cast on one arm still launders its sibling arm - a finer, per-arm key would be needed) and
[[interface-dispatch-argument-ungated-for-code-values]] (interface-method dispatch binds through
the vtable slot's declared signature and never consults this gate at all, cast or bare - a
pre-existing hole, not a regression). Counted from disk per bucket: **2 P1 / 51 P2 / 41 P3 / 7 UI
= 101 total.** The gap between this count and the prior recount's 89 reflects other sessions'
landed/filed work between 2026-08-05 and 2026-08-08 that did not append a recount entry here -
this entry documents only this fix's own delta (-1 P1, +2 P2), not a full reconciliation of the
intervening total.
