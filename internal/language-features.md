...
- **Module system**: `import "file.cb"` for multi-file compilation; `import { "a.cb", "b.cb" };` is a grouped shorthand for several plain imports on one line (each entry routes exactly like a bare `import "x";` - same `.cb`/`.h`/`.c` dispatch, no per-entry `lib`/`define`). Every header import is disk-cached; there is no `cache` clause. `as` is meaningful only on a single-entry group (one alias cannot name many files).
...
- **`new T[n]` into `array<T>`**: a decl-init, direct assignment or `return` whose destination is the core `array<T>` desugars `new T[n]` to `array<T>.init(n)` (one-shot `arrayNewDesugarCtx` armed by `ArmArrayNewDesugar`, consumed in `ParseNewExpression`). `auto x = new T[n]` deduces `array<T>` too (instantiation queued by the `RuleDeclaration` case of `ScanAndQueueGenericTypeUses`; `array.cb` is imported by `runtime.cb`); `T* p = new T[n]` stays the raw escape hatch. `init(n)` zero-fills primitive elements; `init_uninit(n)` does not. A fresh heap array cannot be returned as a bare `T*` / `T[]` (count lost) - only `array<T>`, `move T[]` or `move T*` (both `move` forms carry the hidden count out-param; `ReturnCarriesRawArrayCount` is `Pointer && IsMove`, and a view is a pointer). A counted `move` call result may bind to a `T[]` view (`RejectRawPointerToArrayView` exempts a ledgered `FindRawArrayResult`), and the view local records the count slot (`viewFromCountedCall` in the decl path) so scope exit destructs and frees by count.
...
- `list<T>::add(T value)` / `set(int, T value)` take a PLAIN argument: a value element is deep-copied (the caller's value stays intact), a bare pointer / interface element is aliased, and only a `unique` element type is a sink (a `unique`-typed parameter transfers ownership and nulls the caller's source). `dictionary<K,V>::add/set(K, move V value)` still take `move V` regardless of ownership - a NAMED pointer local passed in is always nulled by the call, even when the target does not end up owning it.
- **`list<T>` element ownership now keys off `is_unique(T)`, not `is_pointer(T)` (BREAKING CHANGE).** A bare `list<T*>` / `list<IShape>` is a **borrowed view** - `add`/`set`/`removeAt`/`clear`/the destructor never free pointer or interface elements. Only `list<unique T*>` / `list<unique IShape>` **owns** its elements: the destructor, `removeAt()`, `clear()`, and overwriting `set()` free them (interface elements via the vtable dtor slot); `[]`/`get(i)` return a borrow (`alias`); `.copy()` on a unique-element list is a compile error ("cannot copy a list of unique elements; use move or copy elements explicitly") - a bitwise copy would double-free. `list<unique T*>` and `list<T*>` are distinct instantiations with no implicit conversion. Code written before this change that relied on bare `list<T*>` owning its elements by default must add `unique` to the element type or it silently stops freeing them.
...
- **Pitfall:** feeding a *borrowed* `list<T*>` / `list<IShape>` from an rvalue (`l.add(new T())`) leaks - the list never frees, and there is no named owner left to free either. Give the list an owner: either a named owning local that outlives it, or a `unique` element type.
...
- **`embed("path")`**: Compile-time intrinsic that folds a file's bytes into a private read-only constant global; the path resolves against the source file's own directory (no `..` escape), the result is `string` or `u8[N]` (N = file size) chosen by the destination type, and the asset is listed in `<out>.cflat-dep.json`.
