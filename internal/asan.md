...
asan can still emit a FRAMELESS report (header only, dies mid-report) when the crash corrupts the heap or happens during teardown - the symbolizer/stack-walk itself faults. Seen on the [[project-threadpool-test-flake]] remaining bugs even with symbolize=0 + fast_unwind. asan is reliable for in-bounds-lifetime UAFs (stored alloc/free stacks) but not for these.
